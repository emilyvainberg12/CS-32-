...
//  Homework 3
//
//  Created by Emily Vainberg on 8/1/20.
...
{
      // Return true iff the vector is sorted according to the ordering relationship compareSensor
...
int main()
{
    ...
    Timer timer;
      ...
      // Sort the Sensors using the STL sort algorithm.  It uses a variant of quicksort called introsort.
    ...
    report("STL sort", elapsed, sensors);
      ...
      // Sort the already sorted array using the STL sort.  This should be fast.
    ...
    {
          // Sort the original unsorted array using insertion sort.  This should be really slow.  If you have to wait more than a minute, try rerunning the program with a smaller number of Sensors.
        ...
        report("insertion sort if not already sorted", elapsed, sensors);
          ...
          // Sort the already sorted array using insertion sort. This should be fast.
    ...
    timer.start();
      ...
      // We create it in a local scope so that we also account for the destruction time.
    {
        
    vector<Sensor> auxSensors(sensors); //given: don't touch
      
    //Create a vector of Sensor pointers, and set each pointer to point to the corresponding Sensor in auxSensors.
    vector<Sensor*> sensorsPtrs;
            
    for (int i=0; i < auxSensors.size(); i++){
        sensorsPtrs.push_back(&auxSensors[i]);
    }
        
    //Sort the vector of pointers using the STL sort algorithm with compareSensorPtr as the ordering relationship.
    sort(sensorsPtrs.begin(), sensorsPtrs.end(), compareSensorPtr);
  
    //Using the now-sorted vector of pointers, replace Sensors in sensors with the Sensors from auxSensors
        for (int i=0; i<sensorsPtrs.size(); i++){
           sensors[i] = *sensorsPtrs[i];
        }
